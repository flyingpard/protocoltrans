...
#ifndef lint
static char rcsid[] =
"$Id: hl7_api.c,v 1.3 1993/03/26 02:56:55 cherniz Exp $";
#endif
...
static int UnmarshallSeg(HL7MSG *, HL7SegRule *, SIZE *);
static int UnmarshallSep(HL7MSG *, char *);
...
static int PoolToWire(HL7MSG *, char *, SIZE);
				/* resolve segment name to ptr to rule/desc. */

HL7SegRule *pSegRule( HL7MSG *pMsg, char *segNm)
{				/* resolve a segment name ("MSH") to
  ...
  while ( strncmp( pSR->azNam, segNm, HL7_SEG_NAME_LEN) !=0) {
    pSR = pSR->nxt;
    if (pSR==0) {
      fprintf( stderr, "pSegRule couldn't find %s\n",segNm);
      return(NULL);
    }
  }
  return pSR;
...
/* This function reads in two files, segDefs and msgRules. The tblPath arg.
   ...
   By Allen Rueter							*/
    ...
    while ( !feof(mrfd) && !ferror(mrfd)) {
      ...
      if ( strlen(evnt) < 3 || strlen(rule) < 3) {
	fprintf( stderr,
	       "HL7Init msgRule ERROR Problem with EVNT=%s RULE=%s fscanf=%d\n",
				evnt, rule, s); 
		return(NULL);
      }
      state=1;
...
HL7MSG * HL7Alloca( HL7FLVR *pF)
{
  ...
  if ((pMsg = (HL7MSG *) calloc(sizeof(HL7MSG),1)) == NULL) {
...
int HL7InsSegm(HL7MSG *pMsg, char *pInsSeg)
{
  HL7SEG *pL;			  /* insert new list item into structure */
  HL7SegRule *pInsSegR;
  pInsSegR = pSegRule( pMsg, pInsSeg);
  if ( pInsSegR == NULL) return (HL7_SEG_NAME_UNDEFINED);
  ...
  if (( pL = (HL7SEG *) calloc (sizeof(HL7SEG),1)) == 0)
    return (HL7_NO_MEMORY);
  
  /* set list link     */
  if (pMsg->pCurrSeg == 0) {	        /* first segment-node               */
    if (pMsg->pSegList != 0) return(HL7_NO_CURRENT_SEG);
...
int HL7WriteMsg(HL7MSG *pMsg, char *pzWire , SIZE MaxWireLen, 
		SIZE *pWireLen)
{
  ...
  if ((pMsg->pCurrSeg = pMsg->pSegList) == 0) return(HL7_EMPTY_STRUCT);
...

{
  ...
  case HL7_WIRE_TO_STRUCT:
    ...
    if ((pSeg = (HL7SEG *) calloc (sizeof(HL7SEG), 1)) == 0)
      return (HL7_NO_MEMORY);
  ...
  case HL7_STRUCT_TO_WIRE:
    ...
    if (pSegR != pMsg->pFlvr->HL7pMSH) {		/* or FHS ? */
      if ((pSeg = pMsg->pCurrSeg->pNextSeg) == 0) return (HL7_MSG_INCOMPLETE);
      if ( pMsg->pCurrSeg->pNextSeg->pSegRul != pSegR) {
	sprintf( pMsg->ErrorMessage, 
		"HL7Eat Seg out of order, found %s, expecting %s",
...
static int UnmarshallSeg (HL7MSG *pMsg, HL7SegRule *pSegRule, SIZE *pSegLen)
{
  ...
  *pSegLen = 0;
  ...
  pcT += HL7_SEG_NAME_LEN;
  
  if (pSegRule == pMsg->pFlvr->HL7pMSH) {		/* or FHS? */
    pMsg->FldSep = *(pcT);
    pMsg->CmpSep = *(pcT+1);			
    pMsg->RepSep = *(pcT+2);	
    pMsg->EscSep = *(pcT+3);	
    ...
    if (*pcT != pMsg->FldSep)  return(HL7_BAD_FLD_SEP);
  ...
  if (pSegRule == pMsg->pFlvr->HL7pMSH) {            /* take care of FHS?  */
...
static int UnmarshallRep(HL7MSG *pMsg, char *pRight) {
  ...
  do { /* until repetition scanning is over */
    pMsg->pCurrCom=0;
    pMsg->pCurrSub=0;
    if ((pT = memchr(pL, pMsg->RepSep, (SIZE)(pRight-pL))) != NULL) 
...
static int UnmarshallComp(HL7MSG *pMsg, char *pRight) {
  ...
  do { /* until component scanning is over */
    ...
    if ((pT = memchr(pL, pMsg->CmpSep, (SIZE)(pRight-pL))) != NULL) 
      pR = pT;
    else
      pR = pRight;
    CompCount++;
...
static int UnmarshallSub(HL7MSG *pMsg, char *pRight) {
  ...
  do { /* until subcomponent scanning is over */
    if ((pT = memchr(pL, pMsg->SubSep, (SIZE)(pRight-pL))) != NULL)
      pR = pT;
    else
      pR = pRight;
    SubCount++;
	...
	fflush(stdout);
      ...
      if ((pSub->pzDataStr = (char *)malloc((SIZE)(pR-pL+1))) == NULL)
	return (HL7_NO_MEMORY);
      DB {
	printf("\nUnmarshallSub :");
	for(i=0; i < (pR-pL); i++)
	...
	fflush(stdout);
      ...
      strncpy(pSub->pzDataStr, pL, (SIZE)(pR-pL));
      pSub->SubLen = pR-pL;
...
static int UnmarshallSep(HL7MSG *pMsg, char *pString) {
  HL7REP *pRep;
  HL7COMP *pComp;
  HL7SUBCOMP *pSubComp;
		 ...
		 int CompNum, int SubNum)
{
  ...
  if (pCompT->CompIndex != CompNum) return(NULL);  
  ...
  do {
    ...
    if (pSubT->SubCompIndex != SubNum) return(NULL);
...
int HL7ReadMsg(HL7MSG *pMsg, char *pzWire)
{
  int i;
  DB printf("\n Read message");

  if (pMsg == NULL) return(HL7_EMPTY_STRUCT); /* HL7MSG was not allocated*/
...
int skipTo(char clsParen, MSGterm **pCurTerm) {
  ...
  while ( (pLclTerm = pLclTerm->nxt)) {	/* assumes no <> or {} */
    ...
    switch ( clsParen) {
	  ...
	  *pCurTerm = pLclTerm->nxt;		/* when recursive skip over */
	  return s;
	}
        break;
      case '>' :
...
static void MsgType2Ecode (HL7MSG *pmsg)
{
  ...
  switch(i) {
    ...
    case HL7_MSGTYPE_LEN:				/* case XXX^YNN	*/
      ...
      break;
    default:                                           /* uknown type    */ 
      sprintf( pmsg->ErrorMessage, "MsgType2Ecode: Bad MsgType %s with length %d",
							 pmsg->cMsgType, i);
      fprintf( stderr, "%s\n", pmsg->ErrorMessage);
      exit(0);
  }
...
/*  printf("\n Marshall -%s", pSegR->azNam; fflush(stdout); */
  ...
  for ( f= (pSegR!=pMsg->pFlvr->HL7pMSH) ? 0 : 1 ; f<FieldAmt; f++) {
	...
	if ((pCompT = pRepT->pComp) != NULL) {
	  if (l!=1)
	    if (PoolToWire(pMsg,&pMsg->RepSep,1) != HL7_OK)
	      return(!HL7_OK);
	  if (pRepT->RepNum >l) {
	    for (j=0; j<(pRepT->RepNum - l);j++) 
	      if (PoolToWire(pMsg,&pMsg->RepSep,1) != HL7_OK)
	  ...
	  do {
	    ...
	    if ((pSubT = pCompT->pSCompLst) != NULL) {
	      if (k!=1)
		if (PoolToWire(pMsg, &pMsg->CmpSep,1) != HL7_OK)
		  return(!HL7_OK);
	      if (pCompT->CompIndex > k) {
		for (j=0; j<(pCompT->CompIndex - k);j++) 
		  if (PoolToWire(pMsg, &pMsg->CmpSep,1) != HL7_OK)
		...
		if (pSubT->pzDataStr != NULL) {
		  if (i != 1)
		    if (PoolToWire(pMsg, &pMsg->SubSep,1) != HL7_OK)
		      return(!HL7_OK);
		  if (pSubT->SubCompIndex > i) {
		    for (j=0; j<(pSubT->SubCompIndex - i);j++) 
		      if (PoolToWire(pMsg, &pMsg->SubSep,1) != HL7_OK)
		  ...
		  if (PoolToWire(pMsg, pSubT->pzDataStr, 
				 pSubT->SubLen) != HL7_OK) 
		    return(!HL7_OK);
		}      
	      } while(pSubT = pSubT->pSubNext);
...
static int PoolToWire(HL7MSG *pMsg, char *pChar, SIZE Len)
{
  if (pMsg->WireLen + Len > pMsg->MaxWireLen) return(!HL7_OK);
...
int HL7IsNextSeg (HL7MSG *pMsg, HL7SegRule *pSegR)
{
  ...
  case HL7_WIRE_TO_STRUCT:
    ...
    break;
  case HL7_STRUCT_TO_WIRE:
    if (pMsg == 0) return (HL7_EMPTY_STRUCT);
...
, char* segName         /* the name of the segment to locate 	*/
)
{
  HL7SEG* pSegT;
  ...
  { 				/* Don't locate this			*/
    pSegT = pSegT->pNextSeg;	/* segment twice  */	
  }
...
{
  if (pMsg == 0) return 0;		/* HL7_EMPTY_STRUCT */
  if (pMsg->pSegList == 0) return 0;	/* HL7_EMPTY_STRUCT */
...
int HL7Free(HL7MSG *pMsg)
{
  ...
  if (pMsg == NULL) return(HL7_EMPTY_STRUCT);
...
int HL7PutFld(HL7MSG *pMsg, char *pString, int FieldNum)
{
  HL7SEG       *pSegT;
  HL7REP       *pRepT,  *pRepN;
  HL7COMP      *pCompT, *pCompN;
  ...
  if (pSegT->pFldLst[FieldNum-1] != 0) { /* field already exists */
  ...
  if (strcmp(pString,HL7_NULL_FLD) && 
      (strlen(pString) !=0)) {	/* regular field            */
 ...
 /* update message type */
    ...
    if ((pMsg->pCurrSeg->pSegRul == pMsg->pFlvr->HL7pMSH) && (pMsg->iCurrFldI0b == 9-1)) {
...
int HL7PutComp(HL7MSG *pMsg, char *pString, int FieldNum)
{
  ...
  if ((pRepT = pSegT->pFldLst[FieldNum-1]) != 0) { 
    ...
    if ((pRepT = (HL7REP *) calloc(sizeof(HL7REP), 1)) == 0)	
      return (HL7_NO_MEMORY);
