...
/* Constants for set and test service flag for numeric field (pcF_...)    */

#define HL7_NULL_FLD		""               /* (null) field        */
...
#define HL7_NO_CURRENT_SEG        0x00000105
#define HL7_BAD_FLDNUM            0x00000106
#define HL7_MSG_INCOMPLETE        0x00000107
/* "System" Errors */
...
typedef struct _HL7flvr {
  MSGrule *p1Rule;			/* pointer to 1st message rule */
  HL7SegRule *p1SegR;
...
typedef struct _HL7SubComp {
     ...
     SIZE SubLen;                               /* length of subcomponent   */
...
/* list of repetition. A field with single occurence is singular case of    */
...
typedef struct _HL7MsgH {		/* HL7 message handle               */
     ...
     char       *pWireCursor;		/* curernt posit in wire            */
     char       ErrorBuffer[ERR_LEN+1]; /* buffer for error output          */
     char       ErrorMessage[ERR_LEN+1]; /* buffer for error message        */
     SIZE       MaxWireLen;		/* max length for wire buffer       */
     SIZE       WireLen;		/* actual Wire Length               */
     char       *pGetBuffer;	        /* buffer for i/o operations        */
     SIZE       GetBufferSize;          /* size of buffer for i/o operat.   */
} HL7MSG;
...
#define ERR_PREFIX "HL7ImEx (%c): "	/* HL7ImEx */
...
static ERRMSGLIST ErrMsgList[]= {
  ...
  { HL7_END_OF_STRUCT,
      "End of message \"%s\"",
      ERR_W, MSG_ERR},
  { HL7_EMPTY_STRUCT ,"Bad HL7 message handle",
      ERR_E, NIL_ERR},
...
int HL7InsSegm(HL7MSG *, char *);
int HL7PutFld(HL7MSG *, char *, int);
...
int HL7WriteMsg(HL7MSG *, char *, SIZE ,SIZE *);
